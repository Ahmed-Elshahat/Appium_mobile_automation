...
"=== QATTA PARALLEL RESULTS ($(Get-Date -Format 'HH:mm:ss')) ===" | Tee-Object -FilePath "$logDir\_qatta_summary.txt"
foreach ($s in $suites) {
  ...
  if (Test-Path $log) {
    ...
    ("{0,-40} {1,-8} {2}" -f $s, $build, $summary) | Tee-Object -FilePath "$logDir\_qatta_summary.txt" -Append
  }
  else { ("{0,-40} NO-LOG" -f $s) | Tee-Object -FilePath "$logDir\_qatta_summary.txt" -Append }
}
Get-Job | Where-Object Name -in $suites | Remove-Job -Force
"=== DONE ===" | Tee-Object -FilePath "$logDir\_qatta_summary.txt" -Append
