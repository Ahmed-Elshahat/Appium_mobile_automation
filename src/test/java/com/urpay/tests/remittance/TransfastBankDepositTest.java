...
/**
 * Transfast International Transfer (Remittance squad) — Bank Deposit.
 *
 ...
 * this route and — unlike Tahweel AlRajhi (H2H) — has NO payout-bank step, so the standard flow
 * applies (the flow's bank gate does not trigger for "Transfast").
 *
 * Migrated from Katalon:
 *   Scripts/Remittance/InternationalTran/Perform International Transfer/
 *     - To Validate Perform Transfast Bank Deposit International Transfer for Indian Beneficiary
 *       (serviceProvider "Transfast", serviceProviderIndex 3, deliveryOption "Bank Deposit",
 *        India / Indian Rupee / 20.50 SAR)
 *
 * Strategy: the login account (transfast.bankDeposit.user.* = 0520956626, poi 2153491614 IQA) is
 * expected to have an active Transfast Bank-Deposit beneficiary; the case searches + selects it. If
 * the account has no such beneficiary the case SKIPS cleanly, and the flow logs the account's
 * beneficiaries so the real name can be pinned in config.
 *
...
public class TransfastBankDepositTest extends BaseTest {

    private static final String OTP_KEY = "transfast.bankDeposit.user.verificationCode";
    private static final String PREFIX = "transfast.bankDeposit";

    @Test(priority = 1, groups = {"remittance", "international-transfer", "transfast",
            "transfast-bankdeposit", "smoke"})
    @Story("Transfast Bank Deposit transfer")
    @Description("Login → International Transfer → search + select the Transfast Bank Deposit "
    ...
    public void testTransfastBankDepositTransfer() {
        ConfigManager c = ConfigManager.getInstance();

        DashboardPage dashboard = new LoginFlow().loginWith(
                c.get("transfast.bankDeposit.user.mobileNumber"),
                c.get("transfast.bankDeposit.user.id"),
                c.get(OTP_KEY, "1234"),
                c.get("transfast.bankDeposit.user.passCode", "2233"));
