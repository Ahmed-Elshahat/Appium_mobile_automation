...
/**
 * Transfast International Transfer (Remittance squad) — Send to Wallet (Philippines).
 *
 * Reuses the operator-agnostic plumbing (InternationalTransferPage / InternationalTransferFlow /
 * InternationalTransferData); Transfast only supplies its data. Transfast is provider card 2 on
 * this Philippines route and has NO payout-bank step (unlike Tahweel/H2H), so the standard flow
 * applies.
 *
 * Migrated from Katalon:
 *   Scripts/Remittance/InternationalTran/Perform International Transfer/
 *     - To Validate Perform Transfast Send To Wallet International Transfer for Philippine
 *       Beneficiary (serviceProvider "Transfast", serviceProviderIndex 2, deliveryOption
 *       "Send to Wallet", Philippines / Philippine Peso / 100.5 SAR, beneficiary "Osama Othman Ali")
 *
 * Strategy: the login account (transfast.sendToWallet.user.*) is expected to have an active
 * Transfast Send-to-Wallet beneficiary; the case searches + selects it. If the account has no such
 * beneficiary the case SKIPS cleanly, and the flow logs the account's beneficiaries so the real
 * name can be pinned in config.
 *
 * ⚠ REAL MONEY: the case moves real funds (amount from config). It drives the flow ONCE to the
 * confirmation screen, validates the confirmation details there (no money yet), and only then
 * confirms — so the beneficiary→confirm steps are NOT executed twice.
 */
@Epic("Remittance")
@Feature("International Transfer - Transfast")
public class TransfastSendToWalletTest extends BaseTest {

    private static final String OTP_KEY = "transfast.sendToWallet.user.verificationCode";
    private static final String PREFIX = "transfast.sendToWallet";

    @Test(priority = 1, groups = {"remittance", "international-transfer", "transfast",
            "transfast-sendtowallet"})
    @Story("Transfast Send to Wallet transfer")
    @Description("Login → International Transfer → search + select the Transfast Send-to-Wallet "
            + "beneficiary → amount → select Transfast (card 2) → purpose → CONFIRMATION page: "
    ...
    public void testTransfastSendToWalletTransfer() {
        ConfigManager c = ConfigManager.getInstance();

        DashboardPage dashboard = new LoginFlow().loginWith(
                c.get("transfast.sendToWallet.user.mobileNumber"),
                c.get("transfast.sendToWallet.user.id"),
                c.get(OTP_KEY, "1234"),
                c.get("transfast.sendToWallet.user.passCode", "2233"));
