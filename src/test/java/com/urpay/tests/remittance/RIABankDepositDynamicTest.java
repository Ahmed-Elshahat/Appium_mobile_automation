...
/**
 * Dynamic RIA Bank Deposit Beneficiary Creation & Transfer — uses real corridor data from MTO DB.
 ...
 *
 * Command:
 *   mvn clean test "-Dsuite=suites/ria-bankdeposit-dynamic.xml" "-Dprofile=sit-remittance"
 *       -Denv=SIT -Dremote=true
 */
@Epic("Remittance")
@Feature("RIA Bank Deposit Dynamic Beneficiary (DB-driven)")
public class RIABankDepositDynamicTest extends BaseTest {
    ...
    public void setupCorridor() throws Exception {
        ...
        if (bankDepositCorridor != null) {
            log.info("RIA Bank Deposit corridor: country={}, currency={}",
    ...
    @Test(priority = 1, groups = {"remittance", "ria-dynamic", "ria-bankdeposit"})
    @Story("Login with fresh registered user")
    @Description("Login with the newly provisioned user to prepare for RIA Bank Deposit beneficiary creation")
    ...
    @Test(priority = 2, dependsOnMethods = "testLogin",
            groups = {"remittance", "ria-dynamic", "ria-bankdeposit"})
    ...
    public void testAddBankDepositBeneficiary() {
        ...
        log.info("RIA Bank Deposit beneficiary '{}' activated successfully", beneficiaryName);
    }
    ...
    public void testTransferToBankDepositBeneficiary() {
        ...
        if (result == InternationalTransferFlow.TransferResult.NO_BENEFICIARY) {
            throw new SkipException("RIA Bank Deposit beneficiary '" + beneficiaryName
